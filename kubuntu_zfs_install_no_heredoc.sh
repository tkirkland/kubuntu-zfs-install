...
#   - ZFS:  RAIDZ1 (disk1-part4 + disk2-part4 + disk3-part2) - all features
#
...
preflight_checks() {
  ...
  if [[ -f /cdrom/casper/filesystem.squashfs ]]; then
    ...
    fatal "Cannot locate filesystem.squashfs. Are you in a Kubuntu live env?"
  fi
...
create_zfs_datasets() {
  ...
  zfs create -o mountpoint=/var/cache \
    -o "com.sun:auto-snapshot=false" "$poolname/var/cache"
  zfs create -o mountpoint=/var/tmp \
    -o "com.sun:auto-snapshot=false" "$poolname/var/tmp"
...
configure_system() {
  ...
  write_lines "$install_root/etc/hosts" \
    "127.0.0.1   localhost" \
    "127.0.1.1   $HOSTNAME" \
    "" \
    "# IPv6" \
  ...
  write_lines "$install_root/etc/apt/sources.list.d/kubuntu.sources" \
    "Types: deb" \
...
chroot_install_inner() {
  ...
  : "${POOLNAME:?POOLNAME is required}"
  : "${USERNAME:?USERNAME is required}"
  : "${DISK1:?DISK1 is required}"
  ...
  # Set the hardware clock to local time (timedatectl doesn't work in chroot)
  write_lines /etc/adjtime \
    "0.0 0 0.0" \
    "0" \
    "LOCAL"
  ...
  # Divert update-grub to prevent ANY invocation during package installations
  # This catches direct postinst calls, triggers, and hook scripts
  dpkg-divert --local --rename \
    --divert /usr/sbin/update-grub.real \
...
DESCRIPTION
  ...
  write_lines /etc/mdadm/mdadm.conf \
  ...
  write_lines /etc/initramfs-tools/conf.d/mdadm \
  ...
  write_lines /etc/apt/preferences.d/no-snapd \
    "Package: snapd" \
    "Pin: release *" \
    "Pin-Priority: -1"
...
https://brave-browser-apt-release.s3.brave.com/ stable main" \
  ...
  write_lines /etc/sudoers.d/nopasswd-apps \
  ...
  write_lines /etc/polkit-1/rules.d/50-nopasswd-packagekit.rules \
  ...
  # Generate MOK key if it doesn't exist (shim-signed creates it on the first DKMS
  ...
  # This prompts for a password now; on the first reboot with Secure Boot enabled,
...
run_in_chroot() {
  ...
  # Enter chroot and run the installation function
  chroot "$install_root" /bin/bash -c \
    "$(declare -f write_lines append_lines chroot_install_inner); chroot_install_inner"
}
...
chroot_install() {
  ...
  unshare --mount --fork --pid --kill-child -- /bin/bash -c \
    "$(declare -f run_in_chroot chroot_install_inner write_lines append_lines); run_in_chroot"
